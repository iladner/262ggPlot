...
#check out hjust (horizontal justification)
p
...
# scatter plot
p2=ggplot(clim, aes(x=tmax, y=tmin))+geom_point(col="blue", shape=3, size=rel(2))
p2
  p2=p2+labs(x="Max Temp C", y= expression(paste("Maximum Temperature ",C**degree))) 
...
p4=ggplot(clim, aes(x=date,y=rain))+geom_line()+ggtitle("Line Graph")
p4
grid.arrange(p,p2,p3,p4, ncol = 4)
```
...
# assuming qualitative
 ...
 p1 = p1 + scale_fill_brewer(type="qual", palette="Oranges")
...
p1 = p1 +theme(axis.text= element_text(face="bold", size=14))
p1 = p1 + scale_color_brewer(type="div", palette="Spectral", name="Thin Intensity", labels=lnms) + 
...
p2= p2+facet_wrap(~as.factor(thin)) 
p2 + scale_color_brewer(type="qual", palette="Dark2", name="Decade") 
p2
